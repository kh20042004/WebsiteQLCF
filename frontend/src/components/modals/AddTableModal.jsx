...
/**
 * AddTableModal Component - Modal để thêm bàn mới
 * Centered modal với form validation
 */
const AddTableModal = () => {
  const { isAddTableModalOpen, closeAddTableModal, showSuccessNotification, showErrorNotification } = useModal();
  ...
  const locationOptions = [
    ...
    { value: 'main_floor', label: 'Tầng Chính' },
    { value: 'window', label: 'Chỗ Cửa Sổ' },
    { value: 'patio', label: 'Sân Vườn Ngoài Trời' },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        showSuccessNotification(`Bàn "${formData.name}" đã được tạo thành công!`);
        handleClose();
      } else {
        showErrorNotification(result.error || 'Có lỗi xảy ra khi tạo bàn');
      }
    } catch (error) {
      console.error('Error creating table:', error);
      showErrorNotification('Có lỗi xảy ra khi tạo bàn');
    } finally {
